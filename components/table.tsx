...
export default function Table(props: { data: Feedback[] }) {
  const columns = React.useMemo<ColumnDef<Feedback>[]>(
    () => [
      ...
      {
        accessorFn: (row) => row.starRating,
        id: 'star_rating',
        cell: (info) =>
          info.getValue() === null ? (
            <span>N/A</span>
          ) : (
            <StarRatings starRating={info.getValue() as number} count={5} />
          ),
        header: () => <span>Star Rating</span>,
        footer: (props) => props.column.id,
      },
      {
...
function Filter({
  ...
  return typeof firstValue === 'number' ? (
    ...
    <input
      className='w-36 border shadow rounded p-1 text-slate-800 font-light'
